...
**Работает в любом окружении** – на обычных серверах, в WSL2, VirtualBox, в облаках без поддержки привилегированных контейнеров.
...
## Подготовка окружения (Ubuntu / Debian / WSL2 / VirtualBox)

Выполните эти команды на чистой виртуальной машине:

```bash
# 1. Обновление пакетов и установка Docker, Git
sudo apt update && sudo apt install -y docker.io docker-compose git

# 2. Добавьте пользователя в группу docker (чтобы не использовать sudo)
sudo usermod -aG docker $USER
newgrp docker

# 3. Проверьте, что Docker установлен
docker --version
```
...
# 2. Выставьте права на приватный ключ (убирает ошибку UNPROTECTED PRIVATE KEY FILE)
...
## Дополнительная настройка для виртуальных машин / WSL2
...
- Состояние VIP-адреса – `MASTER` / `BACKUP` (в режиме с VIP) или отсутствует (в режиме без VIP – можно игнорировать).
